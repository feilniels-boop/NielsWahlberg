...
function renderTestimonials(lang) {
  // Dansk: genbrug forsidens skrevne testimonials (samme #tm-reviews-komponent
  // fra testimonials.js — dansk via `reviews`). Kortene fyldes klientside.
  if (lang === "da") {
    return (
      '<section class="tak-tm"><div class="container">' +
      '<div class="review-grid" id="tm-reviews"></div>' +
      "</div></section>"
    );
  }
  // Engelsk: behold de eksisterende billed-kort (uændret).
...
// Video-testimonials fra forsiden (samme #tm-video-* containere, fyldt af
// testimonials.js). Bruges på den danske takkeside.
function renderVideoTestimonials() {
  return (
    '<section class="clip-section"><div class="container">' +
    '<div class="clip-showcase" id="tm-video-showcase"></div>' +
    '<div class="clip-grid" id="tm-video-grid" aria-label="Kundeclips"></div>' +
    "</div></section>"
  );
}

// Venstre panel: kanal-dashboard (statiske, verificerede tal). Sprog-afhængigt.
function renderDanishPanel(lang) {
  var cfg = channelCfg.danishChannel[lang] || channelCfg.danishChannel.en;
  var lines = cfg.lines
    .map(function (line) {
  ...
  return (
    ...
    '<p class="panel-head">' + esc(cfg.heading) + "</p>" +
...
async function renderPage(req, lang) {
  ...
  // Dansk takkeside henter forsidens testimonials-komponent (fylder #tm-reviews).
  const tmScript = lang === "da" ? '<script src="/testimonials.js"></script>' : "";
  ...
  const panelsSection =
    ...
    renderDanishPanel(lang) +
    renderCalPanel(t) +
    "</div></div></section>";

  // Dansk takkeside: de to paneler (kanal-stats + book et møde) øverst →
  // video-testimonials fra forsiden → skrevne testimonials med stjerner.
  // Engelsk: uændret rækkefølge (billed-kort som før).
  const mainInner =
    lang === "da"
      ? confirmHeader + videoHtml + panelsSection + renderVideoTestimonials() + renderTestimonials("da") + skoolHtml
      : confirmHeader + renderTestimonials("en") + videoHtml + panelsSection + skoolHtml;
