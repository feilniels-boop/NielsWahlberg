...
function renderAbout(c) {
  return (
    ...
    '<div class="avatar" aria-hidden="true">' + esc(c.initials) + "</div>" +
...
function renderClinic(c) {
  ...
  return (
    ...
    // om + initial-cirkel
    ".about{display:grid;grid-template-columns:auto 1fr;gap:22px;align-items:start}" +
    ".avatar{width:72px;height:72px;border-radius:50%;background:var(--accent);color:#fff;display:grid;place-items:center;font-size:1.5rem;font-weight:600;flex:none}" +
    // åbningstider
    ...
    "<p>Vi bruger cookies til at måle og forbedre siden. Du bestemmer.</p>" +
    ...
    "if(a)a.addEventListener('click',function(){s('all');b.hidden=true});" +
