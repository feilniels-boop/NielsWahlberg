...
// Sprogtekster til plan-siden. Sprog udledes af lead.source via claude.leadLang.
const T = {
  en: {
    title: "Your funnel plan | Niels Wahlberg",
    heading: function (name) {
      return name
        ? name + ", here's the funnel I'd build for you."
        : "Here's the funnel I'd build for you.";
    },
    builtLess: "This page was built less than a minute after you filled in the form.",
    builtN: function (n) {
      return "This page was built " + n + " minutes after you filled in the form.";
    },
    noName: "there",
    kSituation: "Your situation",
    kDiagnosis: "The diagnosis",
    kProof: "Proof this works",
    kFunnel: "The funnel I'd build for you",
    kVideos: "Three videos you could film this week",
    kBuildFirst: "What I'd build first",
    kPrice: "Price & next step",
    proofNote:
      "This isn’t theory — it’s how my own channel works. Small reach, the right people, real clients:",
    proofStats: [
    ...
    goal: "A booked call with the right buyer",
    calLead: "Pick a time that works — no back-and-forth:",
    pendingHeading: function (name) {
      return name ? name + ", your plan is being written." : "Your plan is being written.";
    },
    pendingBody:
      "Give it a minute — this page refreshes automatically. Your plan also lands " +
      "in your inbox shortly, with a personal video from me within 48 hours.",
  },
  da: {
    title: "Din funnel-plan | Niels Wahlberg",
    heading: function (name) {
      return name
        ? name + ", her er den funnel jeg ville bygge til dig."
        : "Her er den funnel jeg ville bygge til dig.";
    },
    builtLess: "Denne side blev bygget mindre end et minut efter du udfyldte formularen.",
    builtN: function (n) {
      return "Denne side blev bygget " + n + " minutter efter du udfyldte formularen.";
    },
    noName: "",
    kSituation: "Din situation",
    kDiagnosis: "Diagnosen",
    kProof: "Bevis for at det virker",
    kFunnel: "Tragten jeg ville bygge til dig",
    kVideos: "Tre videoer du kunne optage i denne uge",
    kBuildFirst: "Det jeg ville bygge først",
    kPrice: "Pris & næste skridt",
    proofNote:
      "Det er ikke teori — det er sådan min egen kanal virker. Lav rækkevidde, " +
      "de rigtige mennesker, rigtige kunder:",
    proofStats: [
      { n: "272", label: "median-visninger pr. video" },
      { n: "10+", label: "betalende kunder" },
      { n: "685", label: "abonnenter" },
      { n: "0 kr.", label: "brugt på annoncer" },
    ],
    goal: "Et booket møde med den rigtige køber",
    calLead: "Vælg en tid der passer — ingen frem og tilbage:",
    pendingHeading: function (name) {
      return name ? name + ", din plan er ved at blive skrevet." : "Din plan er ved at blive skrevet.";
    },
    pendingBody:
      "Giv den et minut — siden opdateres automatisk. Din plan lander også i din " +
      "indbakke om lidt, med en personlig video fra mig inden for 48 timer.",
  },
};
function tFor(lead) {
  return T[claude.leadLang(lead)] || T.en;
}
...
function renderFunnel(funnel, t) {
  ...
  const goal =
    ...
    '<div class="plan-flow-card"><strong>' + esc(t.goal) + "</strong></div></li>";
...
function renderProof(t) {
  const tiles = t.proofStats
    ...
    .join("");
  return (
    '<p class="plan-proof-note">' + esc(t.proofNote) + "</p>" +
...
function renderPlanPage(lead) {
  ...
  const t = tFor(lead);
  const lang = claude.leadLang(lead);
  const name = lead.name ? esc(lead.name.split(/\s+/)[0]) : t.noName;
  const parts = [];

  parts.push(renderVideo(lead));

  parts.push(block(t.kSituation, "", "<p>" + esc(plan.situation) + "</p>"));
  parts.push(block(t.kDiagnosis, "", "<p>" + esc(plan.diagnosis) + "</p>"));
  parts.push(block(t.kProof, "", renderProof(t)));
  parts.push(block(t.kFunnel, "", renderFunnel(plan.funnel, t)));
  parts.push(block(t.kVideos, "", renderTitles(plan.video_titles)));
  parts.push(block(t.kBuildFirst, "", "<p>" + esc(plan.build_first) + "</p>"));
  parts.push(
    block(
      t.kPrice,
      "",
      "<p>" + esc(plan.price_and_booking) + "</p>" +
        '<p class="plan-cal-lead">' + esc(t.calLead) + "</p>" +
        '<div class="cal-embed" id="my-cal-inline-30min"></div>'
    )
  );

  const built = builtLine(lead, t);
...
function renderPendingPage(lead) {
  ...
  return (
    ...
    '<div class="plan-wrap"><h1>' + t.pendingHeading(name) + "</h1>" +
    "<p>" + esc(t.pendingBody) + "</p></div></body></html>"
  );
}
