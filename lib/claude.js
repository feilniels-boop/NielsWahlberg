...
// Sikkerhedsnet: fjern lange tankestreger fra genereret tekst. Prompten beder
// også modellen undgå dem, men dette garanterer, at em-dashes ALDRIG slipper
// igennem til læseren (em-dash → komma, en-dash → almindelig bindestreg).
function humanizeDashes(s) {
...
function cleanPlan(parsed) {
  const p = parsed && parsed.plan;
  if (p) {
    ["situation", "diagnosis", "build_first", "price_and_booking"].forEach(function (k) {
      if (typeof p[k] === "string") p[k] = humanizeDashes(p[k]);
    });
    if (Array.isArray(p.funnel)) {
      p.funnel.forEach(function (s) {
        if (s && typeof s.step === "string") s.step = humanizeDashes(s.step);
        if (s && typeof s.detail === "string") s.detail = humanizeDashes(s.detail);
      });
    }
    if (Array.isArray(p.video_titles)) p.video_titles = p.video_titles.map(humanizeDashes);
  }
  if (parsed && Array.isArray(parsed.talking_points)) {
    parsed.talking_points = parsed.talking_points.map(humanizeDashes);
  }
  return parsed;
}
