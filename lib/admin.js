...
// Admin må aldrig indekseres (persondata bag kodeord).
...
function requireAuth(req, res) {
  ...
  if (!expected) {
    sendHtml(res, 500, "<h1>Admin er ikke konfigureret (mangler ADMIN_PASSWORD).</h1>", NOINDEX);
    return false;
  }
...
function banner(q) {
  const ok = {
    ...
    closed: "Markeret som kunde (closed).",
  };
...
function renderPage(lead, opts) {
  ...
  return (
    ...
    '<button type="submit" name="action" value="closed" class="sec">Markér kunde (closed)</button>' +
    "</div></form>" +
    "</body></html>"
  );
}
...
async function handleAdminLead(req, res, id, query) {
  ...
  try {
    ...
    return sendHtml(res, 500, "<h1>Fejl ved opslag</h1>", NOINDEX);
  }
  if (!lead) return sendHtml(res, 404, "<h1>Lead ikke fundet</h1>", NOINDEX);

  if (req.method === "GET") {
    return sendHtml(res, 200, renderPage(lead, { query: query || {} }), NOINDEX);
  }
  ...
  try {
    ...
    return sendHtml(res, 400, "<h1>Ugyldig forespørgsel</h1>", NOINDEX);
  }
  ...
  try {
    if (action === "mail1" || action === "mail2") {
      ...
      await db.updateLead(id, Object.assign({}, patch, { status: "closed" }));
      redirectOk = "closed";
    } else {
  ...
  res.writeHead(303, {
    Location: "/admin/lead/" + encodeURIComponent(id) + "?ok=" + redirectOk,
    "X-Robots-Tag": "noindex, nofollow",
  });
  res.end();
}
